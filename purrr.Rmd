...
Pero ¿qué pasa si queremos construcciones más complejas por grupo? Como por ejemplo si quiséramos el gráfico de masa en gramos vs longitud de la aleta por cada isla
...
penguin_plots <- penguins %>%
  ...
  map(~plotly::ggplotly(qplot(data = .x,
                              ...
                              geom = 'point',
                              xlab = 'Masa en gramos',
                              ylab = 'Lengitud de aleta en mm') + 
                          theme_minimal()))

penguin_plots
```

Y todo esto lo podemos generalizar y empaquetar en una función:

```{r}
#' Crea una lista de gráficos interactivos por grupos
#'
#' Construye un gráfico ggplotly para cada subconjunto de datos especificado 
#' @param datos data.frame
#' @param grupo Caracter que especifica qué columna se usará como el grupo por separar  
#' @param ... Parametros adicionales que pasarle a `qplot`. Ver `?qplot` para mayor detalle
#'
#' @return Una lista de objetos plotly, cada uno correspondiente a un valor unico de 'grupo'  
#'
#' @examples
graficos_por_grupos <- function(datos, grupo, ...){
  lista_tablas <- split(datos, datos[[grupo]])
  
  graficos <- map(.x = lista_tablas, 
                  .f = ~plotly::ggplotly(
                    ~qplot(data = .x, ...) + 
                      theme_minimal()
                    )
...
Nótese cómo se colocan los tres puntos dentro de la llamada de qplot, de forma que se pueden pasar todos los parámetros sin requerir explícitamente ninguno.
...
graficos_por_grupos(datos = penguins, 
                    grupo = 'species', 
                    x = bill_length_mm,
                    xlab = 'Lengitud del pico, en mm',
                    alpha = .8,
                    geom = 'histogram')

# y pelado sí funciona
qplot(data = split(penguins, penguins$island)$Biscoe, 
      x = bill_length_mm,
      alpha = 0.8,
      xlab = 'Lengitud del pico, en mm')
```
