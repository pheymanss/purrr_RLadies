...
Tip de vida: dentro del cuerpo de la funcion usar `ctrl` + `shift` + `R`
...
Este tipo de documentación se llama Roxygen2, y esta es la forma de documentar paquetes en R

## Ambientes en R

Cuando una función se ejecuta, crea su propio ambiente contenido en el ambiente global. La función va a buscar primero los valores dentro de su propio ambiente, y si no existen "saldrá" a buscarlos en los niveles superiores.
...
Por supuesto, para esto es que existe la función `return()`, la cual le "entrega" un objeto del ambiente de la función al ambiente desde donde se llamó la función.
...
Al igual que los vectores atómicos, podemos seleccionar un elemento con `[]`. Con corchetes simples, devuelve una lista de longitud 1 que contiene el elemento indexado
...
Con corchetes dobles ``[[]]`` retorna el elemento contenido de la lista
...
Y el parámetro `max.level` nos ayuda a ver la estructura hasta cierto nivel, para mantenerla legible
```{r}
...
Un *funcional* es una función que recibe una funcion en sus parametros, y devuelve un vector
"En R, las funciones son ciudadanas de primera clase" -> se pueden enviar como parámetros en otras funciones
...
**programación funcional: las funciones también son objetos que pueden entregarse como parámetros a otras funciones**
...
```{r}
fimpura = function(x){
  z <- z + mean(x,y) # usa valores que no son parte del ambiente
    
  z <<- 2 # modifica valores fuera de su ambiente
}
```
...
base::apply(X = 1:10, MARGIN = 1, FUN = sqrt) #does not work
base::sapply(X = 1:10, FUN = sqrt)
?lapply; ?vapply; ?mapply

```

# Introducing the map family of functions --------------------------------------
```{r}

# same but better: purrr::map
